...
import PullToRefresh from '@/components/layout/PullToRefresh'; // Importar PullToRefresh
...
const LocalDashboard = () => {
  ...
  const [isRefreshing, setIsRefreshing] = useState(false); // Estado para PullToRefresh
  ...
  }, [profile, fetchActiveRewards]);

  // Función de refresco para PullToRefresh
  ...
  }, [refreshProfile, refreshData, refreshPrices, fetchActiveRewards]);
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-primary-blue to-purple-600 animate-gradient-move text-text-on-color">
      ...
      <main className="flex-grow">
        <PullToRefresh 
          ...
          loading={isRefreshing || loadingStats || loadingPrices || loadingRewards}
        >
          <div className="w-full max-w-6xl mx-auto p-8 space-y-6 bg-white/80 backdrop-blur-sm rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-6">
              ...
              <Button 
                ...
                disabled={isRefreshing || loadingStats || loadingPrices || loadingRewards}
                variant="outline"
                className="flex items-center gap-2 bg-white text-primary-blue border-primary-blue hover:bg-gray-100"
              >
                <RefreshCw className={`h-4 w-4 ${(isRefreshing || loadingStats || loadingPrices || loadingRewards) ? 'animate-spin' : ''}`} />
                Actualizar Datos
              </Button>
            </div>
